import React from 'react';
import s from './Users.module.css';
import {UsersPropsType} from "./UsersContainer";

export function Users(props: UsersPropsType) {

    if(props.usersPage.users.length === 0) {
        props.setUsers([
            {
                id: 1,
                avatarURL: 'https://sun1.beltelecom-by-minsk.userapi.com/s/v1/ig2/LM5zJ9SgTI9kNPgUIpW4MchmmsKtvjcGcQNeEQh5HpUjJnWQ5nIK4NXQKqlIudlFO6zT87_GqL1HwhakvEevs4gQ.jpg?size=200x200&quality=96&crop=4,4,852,852&ava=1',
                followed: false,
                fullName: 'Vladislav',
                status: 'hello world!',
                city: 'Minsk',
                country: 'Belarus'
            },
            {
                id: 1,
                avatarURL: 'data:image/jpeg;base64,',
                followed: true,
                fullName: 'Polina',
                status: 'hello world!',
                city: 'Minsk',
                country: 'Belarus'},
            {
                id: 1,
                avatarURL: 'https://sun2.beltelecom-by-minsk.userapi.com/s/v1/ig1/awKWHadMlKhrMesJKZyPjQTVAfAmI75Fl0odlvG1-WmGW0OhZ54L7YhLwTwqlBpI05JbW7H3.jpg?size=200x200&quality=96&crop=0,293,1592,1592&ava=1',
                followed: true,
                fullName: 'Ivan',
                status: 'hello world!',
                city: 'Russia',
                country: 'Moscow'},
            {
                id: 1,
                avatarURL: 'https://sun1.beltelecom-by-minsk.userapi.com/s/v1/ig2/5SN7mQ1Vx5Q_LD-TuEHHMnn9VPL1O6VilRlHXycgpsDIN0X9HhhKXCbpRllgKdjceGWQ5mtQtzGpgGl54IX2TVoc.jpg?size=200x200&quality=96&crop=0,0,2159,2160&ava=1',
                followed: true,
                fullName: 'Jenya',
                status: 'hello world!',
                city: 'Minsk',
                country: 'Belarus'},
            {
                id: 1,
                avatarURL: 'https://sun9-32.userapi.com/impg/mXqzF1RlK59N7v-MGXrEOZWl9y4gkCBgxiVYNQ/DwQgmTrAztQ.jpg?size=810x1080&quality=96&sign=cd8dc4ab31554923b8b3959440e4c2e8&type=album',
                followed: false,
                fullName: 'Kostya',
                status: 'hello world!',
                city: 'Kalinkovichi',
                country: 'Belarus'
            }
        ])
    }


    return (
        <div>
            {
                props.usersPage.users.map(u => <div key={u.id} className={s.users_wrapper}>
                    <div className={s.avatar_block}>
                        <div>
                            <img src={u.avatarURL} className={s.avatar}/>
                        </div>
                        <div className={s.btn_follow}>
                            {u.followed
                                ? <button onClick={() => {props.follow(u.id)}}>UnFollow</button>
                                : <button onClick={() => {props.unfollow(u.id)}}>Follow</button>}
                        </div>
                    </div>
                    <div className={s.user_info_block}>
                        <div className={s.name_block}>
                            <div>{u.fullName}</div>
                            <div>{u.status}</div>
                        </div>
                        <div className={s.location}>
                            <div>{u.city}</div>
                            <div>{u.country}</div>
                        </div>
                    </div>
                </div>)
            }
        </div>
    )
}

export default Users;
